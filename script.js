...
let goods = [
  ...
  },
  {
      ...
      product_price: 200,
      store_name: "Zərif Moda",
      store_address: "Bakı şəhəri, Azadlıq prospekti 112",
      id: "Ci47uS6CW7Bg",
  },
  {
...
function getList(arr) {
  ...
  arr.forEach((item) => {
      ...
      li.innerHTML = `
          <p><span>Product name: </span> ${item.product_name}</p>
          <p><span>Product description: </span> ${item.product_description}</p>
          <p class="product-price" data-price="${item.product_price}">
              <span>Product price: </span> ${item.product_price} AZN
          </p>
...
btn1.addEventListener("click",() => {
  ...
  let filtered_goods = goods.filter(item => item.store_name === "Əliyev Moda Mərkəzi");
  getList(filtered_goods);
});
...
btn2.addEventListener("click",() => {
  ...
  btn3.style.backgroundColor = 'white';
  let filtered_goods = goods.filter(item => item.store_name === "Moda Dünyası");
  getList(filtered_goods);
});
...
btn3.addEventListener("click",() => {
  ...
  let filtered_goods = goods.filter(item => item.store_name === "Zərif Moda");
  getList(filtered_goods);
});
